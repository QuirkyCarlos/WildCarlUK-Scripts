...
Citizen.CreateThread(function()
    ...
    while true do
        ...
        if inRange and not shown then
        	shown = true
        	exports['wcukTextUI']:Open('[E] To access the bank', 'darkblue', 'left') 
...
Citizen.CreateThread(function()
    ...
    while true do
        ...
        if NearATM() and not isBankOpened and NearATM() ~= "update" then
            ...
            if IsControlJustReleased(0, 38) then
            	ESX.TriggerServerCallback("wcukBanking:GetPIN", function(pin)
            		if pin then
						...
						exports['wcukNotify']:Alert("BANK", "Head up to a bank to set a PIN code", 5000, 'info')
					end
				end)
            end
        ...
        if inRange and not shown then
        	shown = true
        	exports['wcukTextUI']:Open('[E] To access the ATM', 'darkblue', 'left') 
        elseif not inRange and shown then
        	shown = false
        	exports['wcukTextUI']:Close()
        end
...
function openBank()
	...
	ESX.TriggerServerCallback("wcukBanking:GetPlayerInfo", function(data)
		ESX.TriggerServerCallback("wcukBanking:GetOverviewTransactions", function(cb, identifier, allDays)
			...
			if canAccessSociety then
				ESX.TriggerServerCallback("wcukBanking:SocietyInfo", function(cb)
					if cb ~= nil then
						societyInfo = cb
					else
						local societyIban = Config.IBANPrefix..jobLabel
						TriggerServerEvent("wcukBanking:CreateSocietyAccount", society, jobLabel, 0, societyIban)
						Citizen.Wait(200)
						ESX.TriggerServerCallback("wcukBanking:SocietyInfo", function(cb)
							societyInfo = cb
						end, society)
			...
			ESX.TriggerServerCallback("wcukBanking:GetSocietyTransactions", function(societyTranscb, societyID, societyAllDays)
...
RegisterNUICallback("action", function(data, cb)
		...
		if tonumber(data.value) ~= nil then
			if tonumber(data.value) > 0 then
				if data.window == 'bankmenu' then
					TriggerServerEvent('wcukBanking:DepositMoney', tonumber(data.value))
				elseif data.window == 'societies' then
					TriggerServerEvent('wcukBanking:DepositMoneyToSociety', tonumber(data.value), societyInfo.society, societyInfo.society_name)
				end
			else
				exports['wcukNotify']:Alert("BANK", "Invalid amount", 5000, 'error')
			end
		else
			exports['wcukNotify']:Alert("BANK", "Invalid input", 5000, 'error')
		end
	elseif data.action == "withdraw" then
		if tonumber(data.value) ~= nil then
			if tonumber(data.value) > 0 then
				if data.window == 'bankmenu' then
					TriggerServerEvent('wcukBanking:WithdrawMoney', tonumber(data.value))
				elseif data.window == 'societies' then
					TriggerServerEvent('wcukBanking:WithdrawMoneyToSociety', tonumber(data.value), societyInfo.society, societyInfo.society_name, societyInfo.value)
				end
			else
				exports['wcukNotify']:Alert("BANK", "Invalid amount", 5000, 'error')
			end
	...
	elseif data.action == "transfer" then
		if tonumber(data.value) ~= nil then
			if tonumber(data.value) > 0 then
				ESX.TriggerServerCallback("wcukBanking:IsIBanUsed", function(isUsed, isPlayer)
					if isUsed ~= nil then
						if data.window == 'bankmenu' then
							if isPlayer then
								TriggerServerEvent('wcukBanking:TransferMoney', tonumber(data.value), data.iban:upper(), isUsed.identifier, isUsed.accounts, isUsed.name)
							elseif not isPlayer then
								TriggerServerEvent('wcukBanking:TransferMoneyToSociety', tonumber(data.value), isUsed.iban:upper(), isUsed.society_name, isUsed.society)
							end
						elseif data.window == 'societies' then
							...
							if isPlayer then
								TriggerServerEvent('wcukBanking:TransferMoneyToPlayerFromSociety', tonumber(data.value), data.iban:upper(), isUsed.identifier, isUsed.accounts, isUsed.name, societyInfo.society, societyInfo.society_name, societyInfo.value, toMyself)
							elseif not isPlayer then
								TriggerServerEvent('wcukBanking:TransferMoneyToSocietyFromSociety', tonumber(data.value), isUsed.iban:upper(), isUsed.society_name, isUsed.society, societyInfo.society, societyInfo.society_name, societyInfo.value)
							end
						end
					elseif isUsed == nil then
						exports['wcukNotify']:Alert("BANK", "This IBAN does not exist", 5000, 'error')
					end
				end, data.iban:upper())
			else
				exports['wcukNotify']:Alert("BANK", "Invalid amount", 5000, 'error')
	...
	elseif data.action == "change_iban" then
		if Config.CustomIBANAllowLetters then
			...
			ESX.TriggerServerCallback("wcukBanking:IsIBanUsed", function(isUsed, isPlayer)
				...
				if isUsed == nil then
					TriggerServerEvent('wcukBanking:UpdateIbanDB', iban, Config.IBANChangeCost)
				elseif isUsed ~= nil then
					exports['wcukNotify']:Alert("BANK", "This IBAN is already in use", 5000, 'error')
				end
			end, iban)
		elseif not Config.CustomIBANAllowLetters then
			if tonumber(data.iban) ~= nil then
				...
				exports['wcukNotify']:Alert("BANK", "You can only use numbers on your IBAN", 5000, 'error')
			end
		end
	elseif data.action == "change_pin" then
		if tonumber(data.pin) ~= nil then
			if string.len(data.pin) == 4 then
				TriggerServerEvent('wcukBanking:UpdatePINDB', data.pin, Config.PINChangeCost)
			else
				exports['wcukNotify']:Alert("BANK", "Your PIN needs to be 4 digits long", 5000, 'info')
			end
		else
			exports['wcukNotify']:Alert("BANK", "You can only use numbers", 5000, 'info')
		end
	end
end)

RegisterNetEvent("wcukBanking:updateTransactions")
AddEventHandler("wcukBanking:updateTransactions", function(money, wallet)
	Citizen.Wait(100)
	if isBankOpened then
		ESX.TriggerServerCallback("wcukBanking:GetOverviewTransactions", function(cb, id, allDays)
			trans = cb
			...
			TriggerEvent('wcukBanking:updateMoney', money, wallet)
		end)
	end
end)

RegisterNetEvent("wcukBanking:updateMoney")
AddEventHandler("wcukBanking:updateMoney", function(money, wallet)
	if isBankOpened then
...
RegisterNetEvent("wcukBanking:updateIban")
AddEventHandler("wcukBanking:updateIban", function(iban)
	playerIBAN = iban
	SendNUIMessage({
		action = 'updateiban',
		iban = playerIBAN,
...
RegisterNetEvent("wcukBanking:updateIbanPinChange")
AddEventHandler("wcukBanking:updateIbanPinChange", function()
	Citizen.Wait(100)
	ESX.TriggerServerCallback("wcukBanking:GetOverviewTransactions", function(cbs, ids, allDays)
		trans = cbs
	end)
end)

RegisterNetEvent("wcukBanking:updateTransactionsSociety")
AddEventHandler("wcukBanking:updateTransactionsSociety", function(wallet)
	Citizen.Wait(100)
	ESX.TriggerServerCallback("wcukBanking:SocietyInfo", function(cb)
		ESX.TriggerServerCallback("wcukBanking:GetSocietyTransactions", function(societyTranscb, societyID, societyAllDays)
			ESX.TriggerServerCallback("wcukBanking:GetOverviewTransactions", function(cbs, ids, allDays)
...
function GenerateIBAN()
	...
	while true do
		...
		if isIBanUsed and not hasChecked then
			isIBanUsed = false
			ESX.TriggerServerCallback("wcukBanking:IsIBanUsed", function(isUsed)
				if isUsed ~= nil then
					isIBanUsed = true
	...
	TriggerServerEvent('wcukBanking:SetIBAN', iban)
end