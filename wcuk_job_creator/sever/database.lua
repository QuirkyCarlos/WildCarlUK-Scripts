...
function setupDatabase()
    ...
    if(resName ~= "wcuk_job_creator") then
        print("It would be appreciated using ^5wcuk_job_creator^7 as name of the resource")
    end
    ...
    if(sqlContent) then
        MySQL.Async.execute(sqlContent, {}, function()
            TriggerEvent('wcuk_job_creator:database:ready')
        end)
    else
        TriggerEvent('wcuk_job_creator:database:ready')
    end
