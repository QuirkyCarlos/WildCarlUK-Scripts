...
AddEventHandler('wcuk_job_creator:handcuffPlayer', function(targetId)
    ...
    if (arePlayersClose(playerId, targetId, 2.0)) then
        if (config.handcuffRequireItem) then
            ...
            if (xPlayer and item and item.count > 0) then
                TriggerClientEvent('wcuk_job_creator:arrestConfirmed', playerId, targetId)
            else
...
RegisterNetEvent('wcuk_job_creator:handcuffTarget')
AddEventHandler('wcuk_job_creator:handcuffTarget', function(targetId)
    TriggerClientEvent('wcuk_job_creator:handcuffPlayer', targetId)
end)

RegisterNetEvent('wcuk_job_creator:dragTarget')
AddEventHandler('wcuk_job_creator:dragTarget', function(targetId)
    local playerId = source

    TriggerClientEvent('wcuk_job_creator:dragTarget', targetId, playerId)
end)

RegisterNetEvent('wcuk_job_creator:putincar')
AddEventHandler('wcuk_job_creator:putincar', function(targetId, vehNetId)
    local playerId = source

    TriggerClientEvent('wcuk_job_creator:putincar', targetId, vehNetId)
end)

RegisterNetEvent('wcuk_job_creator:takefromcar')
AddEventHandler('wcuk_job_creator:takefromcar', function(targetId)
    local playerId = source

    TriggerClientEvent('wcuk_job_creator:takefromcar', targetId)
end)
...
RegisterNetEvent('wcuk_job_creator:actions:getVehicleOwner', getVehicleOwner)
...
RegisterNetEvent('wcuk_job_creator:actions:checkIdentity', checkIdentity)