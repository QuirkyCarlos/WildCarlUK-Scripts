        ...
        if(xPlayer.getInventoryItem(itemToRemoveName).count >= itemToRemoveQuantity) then
            if(canPlayerCarry(playerId, itemToAddName, itemToAddQuantity)) then
                ...
                if(isCloseToMarker(playerId, markerId)) then
                    ...
                    TriggerClientEvent('wcuk_job_creator:process:finishedProcessing', playerId, markerId, true)
                    ...
                    TriggerClientEvent('wcuk_job_creator:process:finishedProcessing', playerId, markerId, false)
...
RegisterNetEvent('wcuk_job_creator:process:startProcessing', startProcessing)