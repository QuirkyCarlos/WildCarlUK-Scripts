            ...
            if(canPlayerCarry(playerId, itemName, itemQuantity)) then
                ...
                if(itemTool) then
                    if(itemToolLoseQuantity) then
                        if(xPlayer.getInventoryItem(itemTool).count >= itemToolLoseQuantity) then
                            ...
                            TriggerClientEvent('wcuk_job_creator:harvest:finishedHarvesting', playerId, markerId, false)
                ...
                if(isCloseToMarker(playerId, markerId)) then
                    if(canPlayerCarry(playerId, itemName, itemQuantity)) then
                        ...
                        TriggerClientEvent('wcuk_job_creator:harvest:finishedHarvesting', playerId, markerId, config.allowAfkFarming)
                    else
                        TriggerClientEvent('wcuk_job_creator:harvest:finishedHarvesting', playerId, markerId, false)
...
RegisterNetEvent('wcuk_job_creator:harvestMarkerId')
AddEventHandler('wcuk_job_creator:harvestMarkerId', harvest)