...
RegisterNetEvent("wcuk_job_creator:openGUI", function()
...
function askQuantity(title, type, min, max, cb)
    ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), type, {
        ...
        local quantity = tonumber(data.value)
         ...
         if quantiy and (not max or quantity <= max) and quantity >= min then
            Menu.close()
...
local function saveCurrentSkin()
    TriggerEvent('skinchange:getSkin', function(skin)
...
function setClothes(outfit, saveOutfit)
    ESX.TriggerServerCallback('esx_skin:getPlayerSkin', function(skin)
        TriggerEvent('skinchange:setSkin', skin)
...
local function toggleDuty(markerId)
    ...
    TriggerEvent('wcuk_job_creator:refreshMakers')
end
  ...
  Citizen.CreateThread(function()
    ...
    Citizen.CreateThread(function()
        ...
        elseif(markerType == "teleport") then
            ...
            ESX.TriggerServerCallback('wcuk_job_creator:getMarkerLabel', function(teleportLabel)
...
local function setPedImmutable(ped)
    ...
    SetPedRelationshipBetweenGroups(1, GetHashKey("AMBIENT_GANG_FAMILY"), plyRelationship)
    SetPedRelationshipBetweenGroups(1, plyRelationship, GetHashKey("AMBIENT_GANG_FAMILY"))
...
local function spawnPed(markerData)
    ...
    Citizen.CreateThread(function()
        ...
        local timmeout = GetGameTimer() + 5000
...
local function getMarkers(cb)
    ESX.TriggerServerCallback('wcuk_job_creator:getMarkers', function(markersData)
...
RegisterNetEvent('wcuk_job_creator:refreshMarkers')
AddEventHandler('wcuk_job_creator:refreshMarkers', function() 
...
AddEventHandler('esx:setJob', function(job)
    TriggerEvent('wcuk_job_creator:refreshMarkers')
end)
...
RegisterNetEvent('wcuk_job_creator:startProgressBar', startProgressBar)
...
RegisterNetEvent('wcuk_job_creator:playAnimation', playAnimation)
...
RegisterNetEvent('wcuk_job_creator:notAllowed')
AddEventHandler('wcuk_job_creator:notAllowed', function(acePermission, license, steamId)
    SendNUIMessage({
        action = "notAllowed",
        license = license,
        steamId = steamId,
