...
local function manageGrades(markerId)
    ESX.TriggerServerCallback('wcuk_job_creator:boss:getJobGrades', function(jobGradesSalaries)
        local elements = {}
        ...
        function(data, menu) 
            ...
            if(gradeId) then
                askQuantity(getLocalizedText('boss:new_salary'), "boss_grade_salary", 0, nil, function(quantity)
                    if(quantity) then
                        TriggerServerEvent('wcuk_job_creator:updateGradeSalary', markerId, gradeId, grade, quantity)
                        menu.close()
                    end
                end)
            end
...
local function fireEmployee(markerId, identifier)
    ...
    function(data, menu) 
        ...
        if(action == "yes") then
            TriggerServerEvent('wcuk_job_creator:boss:fireEmployee', markerId, identifier)
        end
...
local function changeGrade(markerId, identifier)
    ...
    ESX.TriggerServerCallback('wcuk_job_creator:retrieveJobRanks', function(grades) 
        local elements = {}
        ...
        function(data, menu) 
            ...
            if(grade) then
                TriggerServerEvent('wcuk_job_creator:boss:changeGradeToEmployee', markerId, identifier, grade)
            end
...
local function openEmployeesManagement(markerId)
    ESX.TriggerServerCallback('wcuk_job_creator:boss:getEmployeesList', function(employees, gradesLabels) 
        local elements = {}
...
local function openRecruit(markerId)
    ...
    ESX.TriggerServerCallback('wcuk_job_creator:boss:getClosePlayersNames', function(closePlayersNames)
        ...
        function(data, menu) 
            ...
            if(playerId) then
                TriggerServerEvent('wcuk_job_creator:boss:recruitPlayer', markerId, playerId)
                menu.close()
            end
        end,
        function(data, menu)
...
function openBoss(markerId)
    ...
    ESX.TriggerServerCallback('wcuk_job_creator:getBossData', function(bossOptions, jobMoney)
        ...
        function(data, menu) 
            ...
            if(action == "withdraw") then
                askQuantity(getLocalizedText("boss:withdraw_amount"), "boss_withdraw", 1, nil, function(quantity)
                    if(quantity) then
                        TriggerServerEvent('wcuk_job_creator:withdrawSocietyMoney', markerId, quantity)
                        openBoss(markerId)
                    end
                end)
            elseif(action == "deposit") then
                askQuantity(getLocalizedText("boss:deposit_amount"), "boss_deposit", 1, nil, function(quantity)
                    if(quantity) then
                        TriggerServerEvent('wcuk_job_creator:depositSocietyMoney', markerId, quantity)
                        openBoss(markerId)
                    end
                end)
            ...
            elseif(action == "wash") then
                askQuantity(getLocalizedText("boss:how_much_to_wash"), "boss_washmoney", 1, nil, function(quantity)
                    if(quantity) then
                        TriggerServerEvent('wcuk_job_creator:washMoney', markerId, quantity)
                    end
                end)
            end
        end,
