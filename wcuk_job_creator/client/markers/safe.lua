local function depositIntoSafe(markerId)
    ESX.TriggerServerCallback('wcuk_job_creator:getPlayerAccounts', function(accounts)
        if(#accounts == 0) then
        ...
        function(data, menu)
            ...
            ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'safe_deposit_dialog', {
                ...
                if quantity and quantity > 0 and quantity <= data.current.money then
                    ...
                    ESX.TriggerServerCallback('wcuk_job_creator:depositIntoSafe', function(isSuccessful)
...
local function withdrawFromSafe(markerId)
    ESX.TriggerServerCallback('wcuk_job_creator:retrieveReadableSafeData', function(safeData)
        if(#safeData == 0) then
        ...
        function(data, menu)
            ...
            ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'safe_withdraw_dialog', {
                ...
                if quantity and quantity > 0 and quantity <= data.current.money then
                    ...
                    ESX.TriggerServerCallback('wcuk_job_creator:withdrawFromSafe', function(isSuccessful)
                        if(isSuccessful) then
