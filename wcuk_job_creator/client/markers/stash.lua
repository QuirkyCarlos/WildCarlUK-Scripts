local function withdrawStash(markerId)
    ESX.TriggerServerCallback('wcuk_job_creator:retrieveStash', function(elements)
        if(#elements == 0) then
        ...
        function(data, menu) 
            ...
            if(item.value) then
                askQuantity(getLocalizedText('quantity'), "stash_take_dialog", 1, item.quantity, function(quantity)
                    ESX.TriggerServerCallback('wcuk_job_creator:stash:takeItem', function(isSuccessful)
...
local function depositStash(markerId)
    ESX.TriggerServerCallback('wcuk_job_creator:getPlayerInventory', function(elements)
        if(#elements == 0) then
            ...
            if(item.value ~= "emptyinventory") then
                ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'stash_deposit_dialog', {
                    ...
                    if quantity and quantity <= item.quantity then
                        ...
                        ESX.TriggerServerCallback('wcuk_job_creator:stash:depositItem', function(isSuccessful)
                            if(isSuccessful) then
