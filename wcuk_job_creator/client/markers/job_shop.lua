local function openJobShopItems(markerId)
    ESX.TriggerServerCallback('wcuk_job_creator:getJobShop', function(items)
        local elements = {}
            ...
            if(itemId) then
                ...
                if(itemType == "item_standard") then
                    askQuantity(getLocalizedText('quantity'), "job_owned_shop_items_quantity", 1, maxItemQuantity, function(quantity)
                        if(quantity) then
                            TriggerServerEvent('wcuk_job_creator:job_shop:buyItem', markerId, itemId, quantity)

                            openJobShopItems(markerId)
                        end
                    end)
                elseif(itemType == "item_weapon") then
                    TriggerServerEvent('wcuk_job_creator:job_shop:buyItem', markerId, itemId, 1)
...
local function openPutOnSale(markerId)
    ESX.TriggerServerCallback('wcuk_job_creator:getSellableStuff', function(elements)
        ...
        function(data, menu)
            ...
            if(maxItemQuantity > 1) then
                askQuantity(getLocalizedText('quantity'), "job_owned_shop_put_on_sale_quantity", 1, maxItemQuantity, function(quantity)
                    if(quantity) then
                        askQuantity(getLocalizedText('item_price'), "job_owned_shop_put_on_sale_price", 1, nil, function(price)
                            if(price) then
                                TriggerServerEvent('wcuk_job_creator:jobShopPutOnSale', itemName, itemType, quantity, price, markerId)
                ...
                askQuantity(getLocalizedText('item_price'), "job_owned_shop_put_on_sale_price", 1, nil, function(price)
                    if(price) then
                        TriggerServerEvent('wcuk_job_creator:jobShopPutOnSale', itemName, itemType, maxItemQuantity, price, markerId)
            ...
            if(itemId) then
                ...
                if(maxItemQuantity > 1) then
                    askQuantity(getLocalizedText('quantity'), "job_owned_shop_items_remove_from_sale_quantity", 1, maxItemQuantity, function(quantity)
                        if(quantity) then
                            TriggerServerEvent('wcuk_job_creator:job_shop:removeFromSale', markerId, itemId, quantity)

                            openRemoveFromSale(markerId)
                        end
                    end)
                else
                    TriggerServerEvent('wcuk_job_creator:job_shop:removeFromSale', markerId, itemId, maxItemQuantity)
                    openRemoveFromSale(markerId)
                end
            end
        end,
        ...
        function(data, menu)
            ...
            if(itemId) then
                ...
                ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'job_owned_shop_items_restock_quantity', {
                    ...
                    if quantity then
                        TriggerServerEvent('wcuk_job_creator:job_shop:addSupplies', markerId, itemId, quantity)
                        menu2.close()
...
function openJobShop(markerId)
    ...
    ESX.TriggerServerCallback('wcuk_job_creator:canSellInThisShop', function(canSell)
        local elements = {
