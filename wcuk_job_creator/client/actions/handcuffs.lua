...
local function arrestAnimation(targetId, targetPed)
    ...
    TriggerServerEvent('wcuk_job_creator:handcuffTarget', targetId)

    isTryingToArrestAnybody = false
end

RegisterNetEvent('wcuk_job_creator:arrestConfirmed')
AddEventHandler('wcuk_job_creator:arrestConfirmed', function(targetId)
...
local function handcuffPlayer(targetId)
    TriggerServerEvent('wcuk_job_creator:handcuffPlayer', targetId)
end

RegisterNetEvent('wcuk_job_creator:handcuffPlayer')
AddEventHandler('wcuk_job_creator:handcuffPlayer', function()
...
RegisterNetEvent('wcuk_job_creator:dragTarget')
AddEventHandler('wcuk_job_creator:dragTarget', function(playerId)
    if(isHandcuffed) then
...
RegisterNetEvent('wcuk_job_creator:putincar')
AddEventHandler('wcuk_job_creator:putincar', function(vehNetId)
    local plyPed = PlayerPedId()
...
RegisterNetEvent('wcuk_job_creator:takefromcar')
AddEventHandler('wcuk_job_creator:takefromcar', function()
    local plyPed = PlayerPedId()
...
RegisterNetEvent('wcuk_job_creator:actions:handcuff', handcuff)

local function drag()
    local targetId = GetPlayerServerId(getClosestPlayerId())
                
    if(targetId > 0) then
        TriggerServerEvent('wcuk_job_creator:dragTarget', targetId)
    else
        notifyClient(getLocalizedText('no_players_nearby'))
    end
end
RegisterNetEvent('wcuk_job_creator:actions:drag', drag)
...
local function putInCar()
    ...
    if(targetId > 0 and closestVehicle > 0) then
        if(closestDistance < 10.0) then
            TriggerServerEvent('wcuk_job_creator:putincar', targetId, VehToNet(closestVehicle))
        else
...
RegisterNetEvent('wcuk_job_creator:actions:putInCar', putInCar)

local function takeFromCar()
    local targetId = GetPlayerServerId(getClosestPlayerId(10.0))

    if(targetId > 0) then
        TriggerServerEvent('wcuk_job_creator:takefromcar', targetId)
    else
        notifyClient(getLocalizedText('no_players_nearby'))
    end
end
RegisterNetEvent('wcuk_job_creator:actions:takeFromCar', takeFromCar)