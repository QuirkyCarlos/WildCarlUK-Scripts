...
local function lockpickCar()
    ...
    if(closestVehicle > 0 and closestDistance < 2.0) then
        if(isVehicleLocked(closestVehicle)) then
            ESX.TriggerServerCallback('wcuk_job_creator:canLockpickVehicle', function(canLockpickVehicle)
...
RegisterNetEvent('wcuk_job_creator:actions:lockpickCar', lockpickCar)