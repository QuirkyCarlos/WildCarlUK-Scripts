local function robPlayer(playerId)
    ESX.TriggerServerCallback('wcuk_job_creator:getTargetPlayerInventory', function(inventory) 
            ...
            if(item.max ~= nil) then
                ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'actions_menu_quantity', {
                    ...
                    if quantity and quantity <= item.max then
                        ...
                        ESX.TriggerServerCallback('wcuk_job_creator:stealFromPlayer', function(isSuccessful) 
...
RegisterNetEvent('wcuk_job_creator:actions:search', search)